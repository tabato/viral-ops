...
def _preview_card_html(video: VideoResult, thumb_src: str, rank: int) -> str:
    ...
    badge_text = html.escape(f"{video.virality_score:.1f}x {video.flames}".strip())
    safe_url = html.escape(video.video_url)
    img_tag = (
...
<a href="{safe_url}" target="_blank" rel="noopener" class="preview-card">
  ...
  <div class="card-body">
    ...
    <div class="card-meta">
      ...
      <span class="view-count">{_fmt_views(video.view_count)} views</span>
    </div>
  </div>
</a>"""
...
.preview-card:hover .thumb-wrap img { transform: scale(1.05); }
...
def generate_gallery(
    ...
    if preview_videos:
        offset = len(results)
        preview_cards = "".join(
            _preview_card_html(v, thumb_map.get(v.video_id, ""), offset + i)
            for i, v in enumerate(preview_videos, 1)
        )
        preview_section = f"""
<div class="grid-wrapper">
  <div class="section-label">More viral outliers &nbsp;&middot;&nbsp; Click to watch</div>
  <div class="grid">
    {preview_cards}
  </div>
</div>"""
